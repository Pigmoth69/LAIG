...
:- http_handler(root(meerkats), prepReplyStringToJSON, []).
:- http_handler(plog(.), serve_files_in_directory(plog), [prefix]).
http:location(plog, root(plog), []).
user:file_search_path(document_root, '.').
user:file_search_path(plog, document_root(plog)).
